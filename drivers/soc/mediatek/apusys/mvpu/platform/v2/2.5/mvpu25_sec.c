...
#include "linux/soc/mediatek/mtk_apu_device.h"
...
void mvpu25_set_sec_log_lvl(int log_lvl)
{
	mvpu_loglvl_sec = log_lvl;
}

bool mvpu25_get_mvpu_algo_available(void)
{
	return mvpu_algo_available;
}

uint32_t mvpu25_get_ptn_total_size(void)
{
	uint32_t ptn_total_size = 0;

	if (mvpu_algo_available == false) {
		pr_info("[MVPU][IMG] %s: mvpu_algo_available is false\n", __func__);
		return 0;
	}

	ptn_total_size = mvpu_algo_img[3];
...
uint32_t mvpu25_get_ptn_size(uint32_t hash)
{
	uint32_t ptn_size = 0;
...
bool mvpu25_get_ptn_hash(uint32_t hash)
{
	uint32_t ptn_total_num = 0;
	int i = 0;
...
uint32_t mvpu25_get_kerbin_total_size(void)
{
	...
	kerbin_total_size = mvpu_algo_img[ker_img_offset/sizeof(uint32_t) + KER_BIN_SIZE_OFFSET];
...
uint32_t mvpu25_get_ker_img_offset(void)
{
	uint32_t offset = 0;
...
void mvpu25_get_ker_info(uint32_t hash, uint32_t *ker_bin_offset, uint32_t *ker_bin_num)
{
	...
	ker_img_total_num = mvpu_algo_img[ker_img_offset/sizeof(uint32_t) + KER_NUM_OFFSET];
...
void mvpu25_set_ker_iova(uint32_t ker_bin_offset, uint32_t ker_bin_num, uint32_t *ker_bin_each_iova)
{
	uint32_t shift = 0;
	uint32_t ker_size_offset = 0;
	int i = 0;
...
void mvpu25_map_base_buf_id(uint32_t buf_num,
...
uint32_t mvpu25_get_saved_session_id(void *session)
{
	uint32_t cnt = 0;
...
uint32_t mvpu25_get_avail_session_id(void)
{
	uint32_t cnt = 0;
	uint32_t session_id = 0xFFFFFFFF;
...
void mvpu25_clear_session(void *session)
{
	...
	for (session_id = 0; session_id < MAX_SAVE_SESSION; session_id++) {
		...
		if (saved_session[session_id] == (uint64_t)session) {
			...
			mvpu25_free_all_hash(session_id);
...
void mvpu25_update_session_id(uint32_t session_id, void *session)
{
#ifdef MVPU_SEC_USE_OLDEST_SESSION_ID
	sess_oldest++;
...
uint32_t mvpu25_get_saved_hash_id(uint32_t session_id, uint32_t batch_name_hash)
{
	uint32_t cnt = 0;
...
uint32_t mvpu25_get_avail_hash_id(uint32_t session_id)
{
	uint32_t cnt = 0;
	uint32_t hash_id = 0xFFFFFFFF;
...
void mvpu25_clear_hash(uint32_t session_id, uint32_t hash_id)
{
	...
	if (hash_pool[session_id]->hash_list[hash_id] != 0) {
		...
		dma_buf_put(hash_pool[session_id]->hash_dma_buf[hash_id]);
...
void mvpu25_free_all_hash(uint32_t session_id)
{
	uint32_t hash_id = 0;
...
int mvpu25_update_hash_pool(void *session,
							bool algo_in_img,
							uint32_t session_id,
							uint32_t hash_id,
...
#ifdef FULL_RP_INFO
int mvpu25_save_hash_info(uint32_t session_id,
						uint32_t hash_id,
						uint32_t buf_num,
						uint32_t rp_num,
						uint32_t *sec_chk_addr,
...
bool mvpu25_get_hash_info(void *session,
						uint32_t batch_name_hash,
						uint32_t *session_id,
						uint32_t *hash_id,
						uint32_t buf_num)
{
	...
	*session_id = mvpu25_get_saved_session_id(session);

	if (*session_id == 0xFFFFFFFF)
		return false;

	*hash_id = mvpu25_get_saved_hash_id(*session_id, batch_name_hash);
...
int mvpu25_replace_img_knl(void *session,
					uint32_t buf_num,
					uint32_t *sec_chk_addr,
					uint32_t *sec_buf_attr,
					uint32_t rp_num,
...
bool mvpu25_set_rp_skip_buf(uint32_t session_id,
							uint32_t hash_id,
							uint32_t buf_num,
							uint32_t *sec_chk_addr,
							uint32_t *sec_buf_attr,
...
int mvpu25_update_new_base_addr(bool algo_in_img,
						bool algo_in_pool,
						uint32_t session_id,
						uint32_t hash_id,
						uint32_t *sec_chk_addr,
...
int mvpu25_replace_mem(uint32_t session_id,
					uint32_t hash_id,
					uint32_t *sec_buf_attr,
					bool algo_in_pool,
					uint32_t *rp_skip_buf,
...
void mvpu25_CopyArgToPrimem(char *dst_ptr, char *src_ptr, int src_size)
{
...
void mvpu25_CheckPrimemArg(char *dst_ptr, int src_size)
{
	int i, j;
...
int mvpu25_replace_kerarg(void *session,
					uint32_t session_id,
					uint32_t hash_id,
					uint32_t kerarg_num,
					uint32_t *sec_chk_addr,
	...
	for (cnt = 0; cnt < kerarg_num; cnt++) {
		...
		for (i = 0; i < primem_num; i++) {
			if ((kerarg_buf_id[cnt] == primem_src_buf_id[i]) &&
				...
				if (mvpu_loglvl_sec >= APUSYS_MVPU_LOG_DBG) {
					pr_info("[MVPU][Sec] mvpu25_CopyArgToPrimem buf[%d]->buf[%d], ker_ofst 0x%08x, dst_ofst (0x%08x + 0x%08x), size 0x%x\n",
							primem_src_buf_id[i],
							primem_dst_buf_id[i],
							kerarg_offset[cnt],
							primem_dst_offset[i],
				...
				mvpu25_CopyArgToPrimem(primem_ptr, kerarg_ptr, kerarg_size[cnt]);

				if (mvpu_loglvl_sec >= APUSYS_MVPU_LOG_ALL)
					mvpu25_CheckPrimemArg(primem_ptr, kerarg_size[cnt]);
...
void mvpu25_get_pool_kreg_iova(uint32_t *kreg_iova_pool,
						uint32_t session_id,
						uint32_t hash_id,
						uint32_t buf_cmd_kreg)
{
...
void mvpu25_region_sort(uint32_t region_num, uint32_t *region)
{
	int i = 0, j = 0;
	uint32_t tmp = 0;
...
int mvpu25_region_info_set(uint32_t buf_num,
							uint32_t *sec_chk_addr,
							uint32_t *sec_buf_size,
							uint32_t *sec_buf_attr,
							...
							bool protect_phase)
{
	...
	//Sorting
	mvpu25_region_sort(buf_io_total, buf_io_addr);
...
int mvpu25_region_merge(uint32_t region_num,
...
int mvpu25_region_mpu_set(uint32_t session_id,
						uint32_t hash_id,
						uint32_t pmu_buff,
						uint32_t buff_size,
						uint32_t *mpu_seg,
						...
						bool protect_phase)
{
	...
	//Sorting
	mvpu25_region_sort(MVPU_MPU_SEGMENT_NUMS, mpu_seg);

	total_mpu_cnt = buf_io_cnt + 1;
	return total_mpu_cnt;
}

int mvpu25_add_img_mpu(void *mvpu_cmd)
{
	...
	mvpu_req = (struct mvpu_request_v25 *)mvpu_cmd;
	...
	if (mvpu_req->mpu_num > MVPU_MPU_SEGMENT_NUMS - 3) {
		...
		mvpu25_region_sort(MVPU_MPU_SEGMENT_NUMS, mvpu_req->mpu_seg);
...
int mvpu25_update_mpu(void *mvpu_cmd,
					uint32_t session_id,
					uint32_t hash_id,
					uint32_t *sec_chk_addr,
					uint32_t *sec_buf_size,
					...
					bool protect_phase)
{
	...
	buf_io_total = mvpu25_region_info_set(buf_num,
	...
	buf_io_total_merged =
			mvpu25_region_merge(buf_io_total, buf_io_addr, buf_io_size, buf_io_addr_merged);
	...
	total_mpu_cnt = mvpu25_region_mpu_set(session_id, hash_id,
...
bool mvpu25_mem_use_iova(uint32_t addr)
{
...
int mvpu25_check_iova(void *session,
					void *cmd,
					uint32_t desc_type,
					uint32_t chk_num,
					uint32_t chk_base,
					uint32_t chk_size)
{
	...
	if (mvpu25_mem_use_iova(chk_base) == false)
		return 0;
	...
	case DESC_TYPE_EDMA:
		...
		for (i = 0; i < 2; i++) {
			...
			if (mvpu25_mem_use_iova(desc_addr) == false)
				continue;
...
int mvpu25_check_batch_flow(void *session,
						void *cmd,
						uint32_t sec_level,
						uint32_t *kreg_kva,
						uint32_t knl_num)
{
	...
	for (i = 0; i < knl_num; i = i + sample) {
		...
		if ((buf_ptr[0] & 0x2) != 0) {
			...
			ret = mvpu25_check_iova(session, cmd, DESC_TYPE_NONE, 0, chk_base, chk_size);
			if (ret != 0) {
		...
		for (j = 0; j < 6; j = j + 3) {
			...
			ret = mvpu25_check_iova(session, cmd, DESC_TYPE_GLSU, chk_num, chk_base, chk_size);
			if (ret != 0) {
			...
			ret = mvpu25_check_iova(session, cmd, DESC_TYPE_EDMA, chk_num, chk_base, chk_size);
			if (ret != 0) {
				pr_info("[MVPU][Sec] [ERROR] KREG[%03d] desc 0x%08x FAIL\n",
					i, chk_base);
				goto END;
...
int mvpu25_load_img(struct device *dev)
{
	int ret = 0;
	...
	ker_img_offset = mvpu25_get_ker_img_offset();
	//ker_img_iova = mvpu_algo_iova + ker_img_offset;

	ptn_img_size = mvpu25_get_ptn_total_size();
	knl_img_size = mvpu25_get_kerbin_total_size();
...
int mvpu25_sec_init(struct device *dev)
{
	int ret = 0;
	uint32_t session_id = 0;
	uint32_t hash_id = 0;
			 ...
			 char *buf)
{
	...
	if (kerbin_total_size == 0)
		kerbin_total_size = mvpu25_get_kerbin_total_size();
	else
...
int mvpu25_sec_sysfs_init(struct kobject *root_dir)
{
