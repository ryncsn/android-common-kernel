	...
	struct dma_buf_attachment *attach)
{
	...
	if (m->type == MDW_MEM_TYPE_SYSTEM_ISP) {
		...
		uint64_t dma_addr = am->dma_addr;
		ret = mdw_apu_mem_map((uint64_t)m->mpriv, am->sid, &dma_addr);
		if (ret) {
			mdw_drv_err("apumem(0x%llx/%u) map fail\n",
				(uint64_t)m->mpriv, am->sid);
			goto free_table;
		}
		am->dma_addr = dma_addr;
	}
	...
	mdw_mem_debug("s(0x%llx) sg(%p) m(%u/%pad/0x%x)\n",
		(uint64_t)m->mpriv,
		&am_attach->sgt,
		am->sid,
...
{
	mdw_mem_debug("type(%u)sid(%u)m(%pad/0x%llx)\n",
	...
	case MDW_MEM_TYPE_SYSTEM_APU:
		if (mdw_apu_mem_free(am->sid))
			mdw_mem_debug("free apumem type(%u)sid(%u)m(%pad/0x%llx) fail\n",
				am->m->type, am->sid,
