...
#define mdw_mem_dma_show(d) \
	mdw_mem_debug("mem(0x%llx/%d/0x%llx/0x%llx/%pad/0x%llx/%d/%ld)(%d)\n", \
		...
		size_t len, struct sg_table *sgt, bool uncached, void **vaddr)
{
	...
	mdw_mem_debug("mdw buf: 0x%lx, len: 0x%zx, nr_pages: %d\n",
		(unsigned long)buf, len, nr_pages);

	if (!pages) {
		mdw_drv_err("No Page 0x%lx, len: 0x%zx, nr_pages: %d\n",
				(unsigned long)buf, len, nr_pages);
		return -ENOMEM;
	}
	...
	*vaddr = va;
	...
	mdw_mem_debug("buf: 0x%lx, len: 0x%zx, sgt: 0x%lx nr_pages: %d va 0x%lx\n",
	...
	enum dma_data_direction dir)
{
	...
	// int attr = attach->dma_map_attrs;	// As Linux kernel 6.6 doesn't have dma_map_attrs
	int attr = 0;
	int ret = 0;
